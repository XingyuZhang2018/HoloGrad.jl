...
begin # plot exact ϕ and B
    ...
    areas = HoloGrad.find_area(layout, layout_end, image_resolution, 2; area_size=0.05)
...
begin # butterfly_flow_decay
    ...
    areas = HoloGrad.find_area(layout, layout_end, image_resolution, 5; area_size=0.01)
...
begin # flow date
    ...
    _, slms_flow_1 = HoloGrad.evolution_slm_flow(layout, layout_end, slm, algorithm; 
                                            interps=10, 
                                            aditers=15,
                                            α = 1,
                                            β = 2,
    ...
    _, slms_flow_2 = HoloGrad.evolution_slm_flow(layout, layout_end, slm, algorithm; 
                                            interps=10, 
                                            aditers=15,
                                            α = 0.5,
                                            β = 1,
...
begin
    ...
    Δϕ_exact = HoloGrad.ϕdiff(slms_exact[2], slms_exact[1])
    ...
    Δϕ_flow = HoloGrad.ϕdiff(slms_flow[2], slms_flow[1])
    ...
    HoloGrad.plot_slms_ϕ_diff!(ax21, slms_exact[1:2]; color = :red)
    ax22 = Axis(fig[2, 2], aspect = 1, xlabel = "position", limits = (nothing, (-pi, pi)), yticks = yticks)
    HoloGrad.plot_slms_ϕ_diff!(ax22, slms_flow[1:2]; color = :red)
