...
begin # flow date
    ...
    layouts_flow, slms_flow = qgh.evolution_slm_flow(layout, layout_end, slm, algorithm; 
                                                          interps=10, 
                                                          aditers=15,
                                                          α = 0.5,
                                                          β = 1,
...
begin # butterfly_flow_all
    ...
    ax1 = Axis(fig[1, 1], xlabel = "steps", ylabel = L"\Delta x/\mathrm{pixel}")
    CairoMakie.lines!(ax1, 1:length(slms_flow), distances)

    ax2 = Axis(fig[1, 2], xlabel = "steps", ylabel = L"B_{\mathrm{max}}")
    CairoMakie.lines!(ax2, 1:length(slms_flow), intensities)

    # display(fig)
    save("project/butterfly_flits_wings/butterfly_flow_all_alpha0.5_beta_1.pdf", fig)
end
